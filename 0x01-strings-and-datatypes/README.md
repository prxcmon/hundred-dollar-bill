...
You can view, compile-and-run, or rewrite my example script of [`strings.c`](strings.c) that uses both functions above.
...
- [`char_array.c`](char_array.c): Defining each characters in each array, one-by-one.
...
- [`char_array2.c`](char_array2.c): The script uses `strcpy` function to _copy_ the string into the `str_a` array without declaring it one-by-one.
